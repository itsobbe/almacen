$(document).ready(function(){  //js que a partir de un codigo puesto en el input codigo va a getDetails hace la consulta y 
 ...
 $(document).on('keydown', '.codigo', function() {
  ...
  var index = 1;
    ...
    document.getElementById('estanteriaVieja').value = "";
    document.getElementById('lejaVieja').value = "";
...
borrado();
  ...
  $( '#'+id ).autocomplete({
   source: function( request, response ) {
    $.ajax({
     url: "../CONTROLADORES/ControladorDevolucionCajaAutocomplete.php",
     type: 'post',
     dataType: "json",
     data: {
   ...
   select: function (event, ui) {
    ...
    document.getElementById('fechaAlta').value = alta;
    document.getElementById('estanteriaVieja').value = estanteria;
    document.getElementById('lejaVieja').value = leja;
